...
void GLWidget::initializeGL()
{
    ...
    m_model.last()->setTranslation(QVector3D(0,0,-2));
    m_model[m_model.size() - 2]->addChild(m_model.last());
    m_transform.append(Transform());
    m_transform.last().translation = QVector3D(0,0,-2);

    m_model.append(new Model());
    m_model.last()->load(":/cube.obj");
    m_model.last()->bind(":/shader.vert", ":/shader.frag");
    m_model.last()->setTranslation(QVector3D(0,0,-3));
    m_model.last()->setRotation(QQuaternion::fromEulerAngles(QVector3D(0,0,0)));
    m_model[m_model.size() - 2]->addChild(m_model.last());
    m_transform.append(Transform());
    m_transform.last().translation = QVector3D(0,0,-3);
    ...
    connect(open, &QAction::triggered, this,
        ...
        transform.translation = QVector3D(0.0f, 0.0f, 0.0f);
        transform.angle = QVector3D(0.0f, 0.0f, 0.0f);
        transform.scale = 1;
        m_transform.append(transform);
