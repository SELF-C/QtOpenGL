...
void GLWidget::updateGL()
{
    ...
    for (int i=0; i < m_model.size(); i++){
        if (i == 0)
        {
            auto light = m_model.first()->getLight();
            m_model.first()->setLight(eye, light.La, light.Ld, light.Ls);

            // モデルの更新処理
            m_model.first()->setTranslation(m_translation.x(), m_translation.y(), m_translation.z());
            m_model.first()->setRotation(m_angle.x(), m_angle.y(), m_angle.z());
            m_model.first()->setScale(m_scale);
        }
    }
